        ...
        switch ($parameters['site']) {
            ...
            case 'faq':
                ...
                $get2 = mysqli_fetch_array(
                    safe_query("SELECT question FROM `" . PREFIX . "faq` WHERE faqID=" . (int)$faqID)
                );
                if ($action == "faqcat") {
                    $returned_title[] = array(
            ...
            case 'forum_topic':
                ...
                if (isset($parameters['topic'])) {
                    ...
                    $boardname = mysqli_fetch_array(
                        safe_query(
                            "SELECT name FROM " . PREFIX . "forum_boards WHERE boardID=" . (int)$topic['boardID']
                        )
                    );
                    $returned_title[] = array(
                        $_language->module['forum'],
            ...
            case 'links':
                ...
                if ($action == "show") {
                    $get = mysqli_fetch_array(
                        safe_query(
                            "SELECT name FROM `" . PREFIX . "links_categorys` WHERE linkcatID=" . (int)$linkcatID
                        )
                    );
                    $returned_title[] = array(
            ...
            case 'news_comments':
                ...
                $message_array = array();
                $query = safe_query(
                    "SELECT n.* FROM " . PREFIX . "news_contents n  WHERE n.newsID=" . (int)$newsID
                );
