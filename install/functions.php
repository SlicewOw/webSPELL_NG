...
function update_40101_420_2($_database)
{
    ...
    while ($ds = mysqli_fetch_array($sql)) {
        if (mysqli_num_rows(mysqli_query($_database, "SELECT userID FROM `" . PREFIX . "user_forum_groups` WHERE `userID`=" . $ds['userID']))) {
          $transaction->addQuery("UPDATE `" . PREFIX . "user_forum_groups` SET `1`='1' WHERE `userID`='" . $ds['userID'] . "'");
        } else {
          $transaction->addQuery("INSERT INTO `" . PREFIX . "user_forum_groups` (`userID`, `1`) VALUES (" . $ds['userID'] . ", 1)");
        }
    }
