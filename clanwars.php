...
<a href="index.php?site=clanwars_details&amp;cwID=' . $cwID . '">' .
            ...
            if (is_array($playerlist)) {
                ...
                foreach ($playerlist as $id) {
                    ...
                    $percpic = '<div class="progress">
                        <div class="progress-bar progress-bar-success" role="progressbar" aria-valuenow="' . round($perc, 0) . '" aria-valuemin="0" aria-valuemax="100" style="width: ' . round($perc, 0) . '%;">
    ...
    $jumpsquads =
     ...
     $jumpmenu =
        '<div class="input-group">
            <select name="selectgame" onchange="MM_jumpMenu(\'parent\',this,0)" class="form-control">
    ...
    if ($type == "ASC") {
        $seiten =
            ...
            ' <span class="bi bi-arrow-down-circle"></span></a> ' . $page_link . '';
    } else {
        $seiten =
            '<a href="index.php?site=clanwars&amp;action=showonly&amp;id=' . $id . '&amp;page=' . $page . '&amp;sort=' .
            $sort . '&amp;type=ASC&amp;only=' . $only . '">' . $_language->module[ 'sort' ] .
            ' <span class="bi bi-arrow-up-circle"></span></a>  ' . $page_link . '';
    }
