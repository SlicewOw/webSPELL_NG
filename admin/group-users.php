...
if (isset($_GET[ 'ajax' ])) {
    ...
    if (isforumadmin($userID)) {
        ...
        if ($action == "usergroups") {
            ...
            $do = safe_query(
                "UPDATE " . PREFIX . "user_forum_groups SET `" . $group . "`='" . $state .
                "' WHERE userID='" . $user . "'"
            );
        } else {
...
if (isset($_GET[ 'action' ])) {
    ...
    if ($pages > 1) {
        echo makepagelink(
            ...
            implode("-", $_POST[ 'users' ]) . "&amp;groups=" . implode("-", $_POST[ 'groups' ]) .
   ...
   <a href="admincenter.php?site=group-users" class="white">'.$_language->module['group_users'].'</a> &raquo; '.$_language->module['edit_group_users'].'<br><br>';
    ...
    for(var x=0;x<document.form.elements.length;x++) {
        ...
        if (y.name!=\'ALL\'){
           ...
           echo '<form method="post" name="form" action="admincenter.php?site=group-users&amp;action=show&amp;users=' .
            implode("-", $_POST[ 'users' ]) . '&amp;groups=' . implode("-", $_POST[ 'groups' ]) . '&amp;addfield=' .
            $_POST[ 'addfield' ] . '">
    ...
    for ($z = $skip; $z < ($skip + $anz_users_page) && $z < $anz_users; $z++) {
        ...
        for ($i = 0; $i < $groups_anz; $i++) {
            ...
            echo '<td class="' . $td . '"><input type="checkbox" onchange="javascript:setUser(\'' .
                $users[ $z ] . '\',\'' . $groups[ $i ][ 'fgrID' ] . '\', this.checked);" value="' .
                $users[ $z ] . ' => ' . $groups[ $i ][ 'fgrID' ] . '"' . $checked . ' /></td>';
        }
        echo '</tr>';
        $n++;
    }
    echo '<tr><td class="td_head">
        ...
        <td class="td_head" colspan="' . ($groups_anz) . '" align="right">
            <input name="grps" type="hidden" value="'. implode(';', $grps) . '" />
            <input name="users" type="hidden" value="' . implode(';', $users) . '" />';
    if ($pages > 1) {
        ...
        $page_select .= '</select>';
        echo $_language->module[ 'save_and_jump' ] . '&nbsp;' . $page_select .
            ' <input name="jump" type="submit" value="' . $_language->module[ 'go' ] . '" /> ';
    }
  ...
  <table class="table table-striped">
    ...
    <tr>
	  ...
	  <tr>
      <td class="td_head" colspan="2" align="right"><button class="btn btn-primary btn-xs" type="submit" />'.$_language->module['show'].'</button></td>
    </tr>
  </table>
  </form>';
}
